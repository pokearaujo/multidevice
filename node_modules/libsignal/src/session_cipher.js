...
class SessionCipher {
    ...
    async decryptPreKeyWhisperMessage(data, keys) {
        assertBuffer(data);
        ...
        return await this.queueJob(async () => {
            ...
            const plaintext = await this.doDecryptWhisperMessageWithKeys(preKeyProto.message, session, keys);
            await this.storeRecord(record);
            if (preKeyId) {
    ...
    async listAllKeys (messageBuffer) {
      let record = await this.getRecord();
      const preKeyProto = protobufs.PreKeyWhisperMessage.decode(messageBuffer.slice(1));
      console.log('prekeyproto', preKeyProto);
      if (!record) {
          if (preKeyProto.registrationId == null) {
              throw new Error("No registrationId");
          }
      ...
      const session = record.getSession(preKeyProto.baseKey);
      return await this.giveMeKeys(preKeyProto.message, session);
    }

    async giveMeKeys(messageBuffer, session) {
      ...
      // this.calculateRatchet(session, message.ephemeralKey, false);
      console.log(message)
      const chain = session.getChain(message.ephemeralKey);
      if (chain.chainType === ChainType.SENDING) {
          throw new Error("Tried to decrypt on a sending chain");
      }
      console.log('pre-fill', chain);
      this.fillMessageKeys(chain, message.counter);
      console.log('post-fill', chain);
      ...
      return {
        ...
        ourIdentityKey,
        keys,
        identityKey2: session.indexInfo.remoteIdentityKey,
      }
    }

    async doDecryptWhisperMessageWithKeys(messageBuffer, session, {
      messageKey,
      ourIdentityKey,
      keys,
      identityKey2
